  ...
  return (
    <div className="home-defaut-page-main min-h-screen bg-slate-300">
        <div className="w-screen h-26 bg-slate-300 flex items-center">
            <article className='text-gray-900 mr-1 text-sm ml-2'>Previous searches:</article>
            {searchFormer.map((element,index)=>(
                <article key={index} className='text-gray-600 mr-1 text-xs'>{element}</article>
            ))}
        </div>
        <div className='w-screen h-auto grid grid-rows-[auto] grid-cols-[auto_auto_auto_auto] '>
            <div className="item-card flex flex-col flex-grow-0 flex-shrink-[.2] h-auto pb-2 w-52 bg-slate-100 shadow-lg rounded-md m-2">
                <img className="w-[60%] h-auto place-self-center mt-1 rounded-sm" src='/Images/Data/3799.jpg' />
                <h3 className='text-gray-900 place-self-center mt-1 text-semibold text-sans text-balance'>Product name</h3>
                <article className='text-gray-600 place-self-center'>$1234.56</article>
                <article className="review text-gray-800 text-balance place-self-center">Rated 4.5 by 154</article>
                <div className='w-full h-auto flex justify-around mt-2'>
                    ...
                    <button className='h-[32px] w-[64px] rounded-lg bg-orange-600 border-none outline-none'>
                        Buy
                    </button>
                </div>
            </div>
